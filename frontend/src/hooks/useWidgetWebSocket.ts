...
export function useWidgetWebSocket({
  ...
  const connect = useCallback(() => {
    ...
    const url = `${WS_BASE}/api/v1/widget/sessions/ws?org_id=${encodeURIComponent(orgId)}`;
  ...
  }, [sessionId, orgId]); // callbacks removed from deps — using refs
